...
ts_age <- read.csv("/home/alice/Dropbox/GAM for demographic projection/Data/Age Category Time Series.csv", sep = ",", header = TRUE, stringsAsFactors = FALSE)
ts_age$X <-NULL
ts_age <- ts_age[which(ts_age$Month < 13),]
#$ts_age$Agecat <- cut(ts_age$Age, c(0,5,15,25,35,45,55,65,150), labels = c("<5","5-14","15-24","25-34","35-44","45-54","55-64",">65"), right=FALSE)

#Age Category Data
#ts_a <-aggregate(data=ts_age, x ~ Agecat+Year+Month, FUN=sum, na.rm=TRUE)
#ts_a$Date <- as.Date(paste("1", ts_a$Month, ts_a$Year, sep='-'), format='%d-%m-%Y') 
#ts_a <- ts_a[order(as.Date(ts_a$Date, format="%d/%m/%Y")),]
...
#pdf(file = "/home/alice/Dropbox/Fiscal Impact/Latex/decomposition.pdf", paper = "special", height = 4, width = 8)
dec <- stl(ts1, t.window=100, s.window=7, robust=TRUE)
plot(dec)
#plot(decompose(ts1, type="additive"), family='serif') # Trend and seasonality
#dev.off()
#dec <- decompose(ts1, type="additive")
#ts$random <- dec$random

ts_dec <- as.data.frame(dec$time.series)
ts$seasonal_dec <- ts_dec$seasonal

#test for residual autocorrelation
#residual <- ts(ts_dec$remainder)
#Box.test(residual, lag = 1) # a small p-value is evidence against independence
# 	Box-Pierce test
#  X-squared = 58.829, df = 1, p-value = 1.721e-14
#auto.arima(residual) # ARIMA(3,0,3)
...
#pdf(file = "/home/alice/Dropbox/GAM for demographic projection/R files/Paper/weak_priors1.pdf", paper = "special", height = 3.5, width = 6)
#pdf(file = "/home/alice/Dropbox/Thesis/Latex/weak_priors1.pdf", paper = "special", height = 3.5, width = 6)
...
write.csv(test, "/home/alice/Dropbox/GAM for demographic projection/Data/Results Bayesian 2006.csv")
...
write.csv(test, "/home/alice/Dropbox/GAM for demographic projection/Data/Results Bayesian 2006 ar1.csv")
...
prediction <- read.csv('/home/alice/Dropbox/GAM for demographic projection/Data/Prediction time series.csv', sep = ",", header = TRUE, stringsAsFactors = FALSE)
#New Variables
...
#ts_a$Agecat <- cut(ts_a$Age, c(0,20,40,60,150), labels = c(1:4), right=FALSE)
#ts_a$Age_cat[ts_a$Agecat==1] <- "0-19"
#ts_a$Age_cat[ts_a$Agecat==1] <- "0-19"
#ts_a$Age_cat[ts_a$Agecat==2] <- "20-39"
#ts_a$Age_cat[ts_a$Agecat==3] <- "40-59"
#ts_a$Age_cat[ts_a$Agecat==4] <- "+60"
...
future_age <- read.csv('/home/alice/Dropbox/GAM for demographic projection/Data/Prediction Age Time Series')
